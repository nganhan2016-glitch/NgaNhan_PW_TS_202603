...
kiemTraMatKhau("Neko@123").then(
  (data) =>  {
    console.log(data);
  }
);

kiemTraMatKhau("Neko").catch(
  (data) =>  {
    console.log(data);
  }
);
...
async function muaHang(url) {
  console.log("bat dau mua hang");
  await moTrangWeb(url);
  let token = await dangNhap(url, "nga", "nga@123");
  await themVaoGioHang(token, sanPham);
  await thanhToan(gioHang);
  console.log("ket thuc mua hang");
}
